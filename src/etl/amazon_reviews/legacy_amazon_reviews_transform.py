...
def cut_reviews_to_max_words(
    ...
    df[review_text_columnn] = df[review_text_columnn].apply(cut_review_text)
...
def save_chunk(df, datapath:str=None, outdir:str=None) -> None:
    """
    Saves a loaded chunk/df, named based on `datapath`.

    Either `datapath` or `outdir` must be specified.
    """
    ...
    if datapath:
        ...
        print("WARNING: datapath or outdir must be specified, df was saved to 'backup'")