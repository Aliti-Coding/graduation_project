...
def create_homepage():
    with engine.connect() as conn:
        stmt = text("SELECT news_api_id, title, source, mean_preds FROM news_api_w_preds limit 10;")
...
def create_news_article(id, conn):
    stmt = text(f"SELECT * FROM news_api_w_preds WHERE news_api_id = {id} limit 1;")
